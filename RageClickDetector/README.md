...
Developed by **Dorian D. Regester**, a Digital Analytics Professional and Implementation Engineer, this tool is designed for enterprise-grade data collection environments. While optimized for **Adobe Analytics (appMeasurement & WebSDK)**, the logic is architected to be platform-agnostic for any modern Tag Management System (TMS).
...
*   **Modification:** Update the `triggerRageClick` function to utilize `alloy("sendEvent", ...)` instead of `s.tl()`.
*   **XDM Mapping:** Map the generated identifier to your XDM schema under `_experience.analytics.customDimensions.eVars.eVar77`.
